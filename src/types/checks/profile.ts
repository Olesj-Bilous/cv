...
//export const isIconicListChecksFirst = (value: any): value is DisplayList<Period> => isListOfTypeChecksFirst(value, isIconicItem);
...
//export const isPeriodListChecksFirst = (value: any): value is DisplayList<Period> => isListOfTypeChecksFirst(value, isPeriod);
...
//export const isRatedSkillListChecksFirst = (value: any): value is DisplayList<RatedSkill> => isListOfTypeChecksFirst(value, isRatedSkill);
...
const isListOfTypeChecksFirst = function <T>(value: any, checker: (val: any) => val is T) {
  return value != null && 'title' in value && 'items' in value && Array.isArray(value.items) && checker(value.items[0]);
}
